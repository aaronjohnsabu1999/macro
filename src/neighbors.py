from ifmea        import listPointToTwoDimPoint, twoDimPointToListPoint
...
def neighborsConfig(config, fConfig):
  ...
  for i in range(numAgents):
    ...
    N[i] = [fConfig[twoDimPointToListPoint(LayerLens,
                                           presentLayerNum,
                                          (presentPositionNum-1)%LayerLens[presentLayerNum])], 
            fConfig[twoDimPointToListPoint(LayerLens,
                                           presentLayerNum,
                                          (presentPositionNum+1)%LayerLens[presentLayerNum])]]
    if presentLayerNum != len(LayerLens) - 1:
      N[i].append(fConfig[twoDimPointToListPoint(LayerLens,
                                                 presentLayerNum+1,
                                                (presentPositionNum*2)-1)])
      N[i].append(fConfig[twoDimPointToListPoint(LayerLens,
                                                 presentLayerNum+1,
                                                (presentPositionNum*2)+0)])
      N[i].append(fConfig[twoDimPointToListPoint(LayerLens,
                                                 presentLayerNum+1,
                                                (presentPositionNum*2)+1)])
...
def neighborsDistance(Neighbors, Nghradius, R_0):
  for egoCount, r_0 in enumerate(R_0):
    for j, r_1 in enumerate(R_0[egoCount:]):
      nghCount = egoCount + j
      if norm(subtract(r_0, r_1)) < Nghradius: # and not(j == egoCount):
        value = 1
      else:
        value = 0
